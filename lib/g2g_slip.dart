...
class GtoG extends StatefulWidget {
  ...
  final String? userId;
  GtoG({super.key, required this.userType, required this.userId});
  @override
...
class GtoGPage extends StatefulWidget {
  final String? userType;
  final String? userId;
  GtoGPage({super.key, required this.userType, required this.userId});
...
class _GtoGPageState extends State<GtoGPage> {
  ...
  Future<void> fetchRegistrationData() async {
    try {
      final url = Uri.parse(
          'http://mybudgetbook.in/GIBAPI/searchbarfetch.php?userId=${widget.userId}');
      final response = await http.get(url);
      ...
      if (response.statusCode == 200) {
        ...
        throw Exception(
            'Failed to load registration data: ${response.statusCode}');
      }
    } catch (error) {
      // Handle other errors
  ...
  Future<void> postData() async {
    ...
    final response = await http.post(url,
        body: jsonEncode({
          ...
          "company_name":
              mycomapny.toString(), // Extract text from TextEditingController
                ...
                Padding(
                  ...
                  child: TypeAheadFormField(
                    ...
                    itemBuilder: (context, dynamic suggestion) {
                      return ListTile(
                        title: Text(
                            '${suggestion['first_name']} (${suggestion['member_id']})'),
                    ...
                    SizedBox(
                      ...
                      child: TextFormField(
                          ...
                          onChanged: (value) {
                            String capitalizedValue =
                                capitalizeFirstLetter(value);
                            metcompanyname.value =
                                metcompanyname.value.copyWith(
                              text: capitalizedValue,
                              selection: TextSelection.collapsed(
                                  offset: capitalizedValue.length),
                            );
