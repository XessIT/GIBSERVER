...
class _HomepageState extends State<Homepage> {
  ...
  void initState() {
    ...
    print("print idd7777777777777${memberId}");
        ...
        if (responseData is List<dynamic>) {
          setState(() {
            userdata = responseData.cast<Map<String, dynamic>>();
             ...
             print("District6666 $district");
            print("chapter99999   $chapter");
  ...
  Future<void> registerFetch(String meetingId) async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (responseData is List<dynamic>) {
          final responseData = json.decode(response.body);
          meeting = responseData.cast<Map<String, dynamic>>();
        } else {
          print('Invalid response data format');
        }
      } else {
  ...
  bool isUserRegistered(String meetingId) {
    for (var data in registerFetchdata) {
      if (data['meeting_id'] == meetingId && data['status'] == 'Register') {
        return true;
      }
    }
    return false;
  ...
  Future<void> getData() async {
    ...
    try {
      final url = Uri.parse(
          'http://mybudgetbook.in/GIBAPI/non_exe_meeting.php?member_type=$type&district=$district&chapter=$chapter');
      print('URL: $url');
      final response = await http.get(url);
      if (response.statusCode == 200) {
        final responseData = json.decode(response.body);
        ...
        List<dynamic> filteredData = itemGroups.where((item) {
          ...
          print('Registration memberID: $memberId');
                      ...
                      child: Row(
                        children: [
                          Padding(
                            ...
                            child: CircleAvatar(
                              radius:
                              28.0, // Adjust the radius to achieve the desired size
                              backgroundImage:
